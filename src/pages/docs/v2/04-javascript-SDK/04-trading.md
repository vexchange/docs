...
const pair = await Fetcher.fetchPairData(DAI, WVET[DAI.chainId])

const route = new Route([pair], WVET[DAI.chainId])

const amountIn = '1000000000000000000' // 1 WVET

const trade = new Trade(route, new TokenAmount(WVET[DAI.chainId], amountIn), TradeType.EXACT_INPUT)
```
...
const path = [WVET[DAI.chainId].address, DAI.address]
